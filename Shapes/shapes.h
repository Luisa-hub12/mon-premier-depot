 ...
 */
circle* create_circle(int x, int y, int radius, const char* color_line, const char* background_color, int thickness); //création d'un cercle
 ...
 * @param dy Déplacement vertical
 */
void move_circle(circle* cir, int dx, int dy); //déplacement du cercle
...
void free_polyline(polyline* pln); // on libère la mémoire da polyligne
...
typedef struct {
    ...
    int width;                 // largeur
    int height;                // hauteur
    char line_color[15];       // couleur du contour (max 14 caractères + '\0')
    char background_color[15]; // couleur de remplissage
    int thickness;             // épaisseur du trait
} rectangle;

// 🔹 Crée un nouveau rectangle avec les paramètres donnés
rectangle* create_rectangle(int x, int y, int width, int height,
                            const char* line_color,
                            const char* background_color,
                            int thickness);

// 🔹 Libère la mémoire allouée pour un rectangle
